...
public class NotificationService extends Service {
	...
	public void onStart(Intent intent, int i) {
		Log.v("eldar", "NotificationService: onStart");
		...
		Log.v("eldar", "NotificationService: started");
	}
	...
	private void scheduleNotificationsOrStop() {
		if (scheduleNotifications() == 0) {
			Log.v("adalx", "Nothing scheduled: Stop self");
			stopSelf();
		}
	}
	...
	public void onDestroy() {
		Log.v("eldar", "NotificaitonService: onDestroy");
		super.onDestroy();
		/*if (manager != null)
			manager.cancelAll();*/
		if (timer != null)
			timer.cancel();
		Log.v("eldar", "NotificationService: destroyed");
	}
	...
	private int scheduleNotifications() {
		...
		Log.v("adalx", "_________________________");
		int count = 0;
		for (OccuredNotification occuredNotification : occuredNotifications) {
			Log.v("adalx", "Missed notification: " + occuredNotification.courseName + "\t"
					+ database.getOccuredNotificationTable().getTimeFormat().format(occuredNotification.occurTime));
		}
			...
			if (!endDateCalendar.before(currentTimeCalendar) || (course.pillsRemained > 0)) {
				...
				if (pillNotification.dayOfWeek == Calendar.getInstance().get(
					...
					if (!pillNotificationCalendar.before(currentTimeCalendar)) {
						...
						Log.v("adalx", "Notification: " + pillNotification.courseName + "\t"
								+ new SimpleDateFormat("dd.MM.yyyy HH:mm").format(pillNotificationDateTime));
