...
public class PillNotificationTable extends AbstractTable {
	...
	public static final String TABLE_ATTRIBUTES = TIME + " TIME NOT NULL,"
			+ COURSE_NAME + " INTEGER NOT NULL," + DAY_OF_WEEK
			+ " INTEGER NOT NULL," + PILLS_TO_TAKE
			+ " INTEGER NOT NULL DEFAULT " + PILLS_TO_TAKE_DEFAULT + ","
			+ "PRIMARY KEY(" + TIME + "," + COURSE_NAME + "," + DAY_OF_WEEK
			+ ")," + "FOREIGN KEY(" + COURSE_NAME + ") REFERENCES "
			+ CourseTable.TABLE_NAME + " ON DELETE CASCADE";
	...
	// returns SimpleDateFormat used in table to convert time from Date to
	...
	public int updatePills(String courseName, int dayOfWeek, Date time,
			int newPillsToTake) {
		int result;
		ContentValues contentValues = new ContentValues();
		contentValues.put(PILLS_TO_TAKE, newPillsToTake);
		try {
			result = db.update(TABLE_NAME, contentValues, COURSE_NAME + "="
					+ "'" + courseName + "'" + " AND " + DAY_OF_WEEK + "="
					+ dayOfWeek + " AND " + TIME + "=" + "'"
	...
	public void delete(String courseName, int dayOfWeek) {
		try {
			deleteWhere(DAY_OF_WEEK + "=" + dayOfWeek + " and " + COURSE_NAME
					+ "='" + courseName + "'");
		} catch (SQLException e) {
		}
	}
	...
	public void delete(String courseName, int dayOfWeek, Date time) {
		try {
			deleteWhere(TIME + "='" + timeFormat.format(time) + "' and "
					+ DAY_OF_WEEK + "=" + dayOfWeek + " and " + COURSE_NAME
					+ "='" + courseName + "'");
		} catch (SQLException e) {
		}
	}
	...
	public void delete(PillNotification notification) {
		delete(notification.courseName, notification.dayOfWeek,
				notification.time);
	}

	// String[] array representing all columns in PillNotificationTable database
	// table
	public static final String[] ALL_COLUMNS = { COURSE_NAME, DAY_OF_WEEK,
			TIME, PILLS_TO_TAKE };
	...
	public ArrayList<PillNotification> selectAll() {
		Cursor cursor = null;
		try {
			cursor = db.query(TABLE_NAME, ALL_COLUMNS, null, null, null, null,
					TIME);
		} catch (SQLException e) {
		}
	...
	// returns all notifications from database which belong to course:
	// courseName
	public ArrayList<PillNotification> select(String courseName) {
		Cursor cursor;
		try {
			cursor = db.query(TABLE_NAME, ALL_COLUMNS, COURSE_NAME + "='"
					+ courseName + "'", null, null, null, TIME);
		} catch (SQLException e) {
			cursor = null;
		}
		ArrayList<PillNotification> result = convertIntoPillNotificationList(cursor);
	...
	// returns all notifications from database which belong to course:
	// courseName, and day of week: dayOfWeek
	public ArrayList<PillNotification> select(String courseName, int dayOfWeek) {
		Cursor cursor;
		try {
			cursor = db.query(TABLE_NAME, ALL_COLUMNS, COURSE_NAME + "='"
					+ courseName + "' AND " + DAY_OF_WEEK + "=" + dayOfWeek,
					null, null, null, TIME);
		} catch (SQLException e) {
			cursor = null;
		}
	...
	public PillNotification selectOne(String courseName, int dayOfWeek,
			Date time) {
		Cursor cursor = null;
		try {
			String where = TIME + "='" + timeFormat.format(time) + "' and "
					+ DAY_OF_WEEK + "=" + dayOfWeek + " and " + COURSE_NAME
					+ "='" + courseName + "'";
			cursor = db.query(TABLE_NAME, ALL_COLUMNS, where, null, null, null,
					TIME);
			cursor.moveToFirst();
		} catch (SQLException e) {
		}
	...
	private ArrayList<PillNotification> convertIntoPillNotificationList(
			Cursor cursor) {
