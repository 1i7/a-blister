...
public class OccuredNotificationTable extends AbstractTable {
	...
	public static final String TABLE_ATTRIBUTES = COURSE_NAME
			+ " INTEGER NOT NULL," + OCCUR_TIME + " INTEGER NOT NULL,"
	...
	public void delete(String courseName, Date occurTime) {
		try {
			deleteWhere(OCCUR_TIME + "='" + timeFormat.format(occurTime)
	...
	public ArrayList<OccuredNotification> select(String courseName) {
		...
		try {
			cursor = db.query(TABLE_NAME, ALL_COLUMNS, COURSE_NAME + "='"
					+ courseName + "'", null, null, null, OCCUR_TIME);
		} catch (SQLException e) {
			cursor = null;
		}
		ArrayList<OccuredNotification> result = convertIntoOccuredNotificationList(cursor);
	...
	private ArrayList<OccuredNotification> convertIntoOccuredNotificationList(
			Cursor cursor) {
		...
		while (!cursor.isAfterLast()) {
			...
			try {
				occuredNotification.occurTime = timeFormat.parse(cursor
						.getString(occurTimeIndex));
